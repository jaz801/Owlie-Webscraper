...
def main():
    url = 'https://www.cloudway.be/jobs#jobs'  # Updated URL

    try:
        driver = get_driver()
        driver.get(url)

        h5_elements = driver.find_elements(By.CSS_SELECTOR, 'h5.py-8')  # Find <h5> elements with the specified class
        stored_items = [h5.text.strip() for h5 in h5_elements]
