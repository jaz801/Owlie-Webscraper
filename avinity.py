...
def main():
    ...
    try:
        ...
        h2_elements = driver.find_elements(By.CLASS_NAME, 'entry-title')  # Find all <h2> elements with class "entry-title"
        stored_items = []
        ...
        # Change row from 19 to 20
        row_number = 20

        worksheet.update_cell(row_number, 1, url)  # Updated to row 20
        for i, item in enumerate(stored_items, start=1):
            worksheet.update_cell(row_number, i + 1, item)  # Updated to row 20 if item is not empty
            print(f"Added item {i}: {item}")

        amsterdam_tz = pytz.timezone('Europe/Amsterdam')
        current_datetime_amsterdam = datetime.now(amsterdam_tz)

        worksheet.update_cell(row_number, len(stored_items) + 2, current_datetime_amsterdam.strftime("%Y-%m-%d"))  # Updated to row 20
        worksheet.update_cell(row_number, len(stored_items) + 3, current_datetime_amsterdam.strftime("%H:%M:%S"))  # Updated to row 20
