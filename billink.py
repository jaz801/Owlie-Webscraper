...
def main():
    url = 'https://www.billink.nl/werkenbij'  # Updated URL

    try:
        driver = get_driver()
        driver.get(url)

        span_elements = driver.find_elements(By.CSS_SELECTOR, 'span[style="color: #f28d20; font-size: 20px;"]')  # Find all <span> elements with the specified style
        stored_items = []
