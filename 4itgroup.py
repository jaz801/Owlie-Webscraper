...
def main():
    ...
    url = 'https://www.4itegogroup.com/nl/jobs#job-list'
    ...
    h4_elements = soup.find_all('h4', class_="mt-6 mb-2 text-primary")
    ...
    current_datetime = datetime.now(amsterdam_timezone)
    ...
    row_index = 2 # Start with row 2 (index 1) to skip header row

    # Write the data to the sheet
    worksheet.update_cell(2, 1, 'https://www.4itegogroup.com/nl')
    for i, job in enumerate(h4_elements, start=1):
        job_text = job.get_text()  # Extract the text content of the h4 element
        worksheet.update_cell(2, i+1, job_text)
        print(f"Added job {i}: {job_text}")

    # Update the adjacent cells with the current date and time
    worksheet.update_cell(2, len(h4_elements) + 2, current_date)
    worksheet.update_cell(2, len(h4_elements) + 3, current_time)
    print(f"Updated cell: {len(h4_elements) + 2}, {current_date}")
    print(f"Updated cell: {len(h4_elements) + 3}, {current_time}")
