...
def main():
    ...
    try:
        ...
        for _ in range(20):  # Run the loop a maximum of 20 times
            if not div_elements:
                break
            
            div = div_elements.pop(0)
            div_text = div.text.strip()
            
            if div_text:
                if '.' in div_text:
                    continue
                if 'Slim genoeg om het simpel te houden' in div_text:
                    continue
                if any(phrase in div_text for phrase in [
                    'Ga naar vacature',
                    'Ⓒ BPI Services',
                    'Contact ons',
                    'Over ons',
