...
def main():
    url = 'https://join.accogroup.be/#jobs'
    ...
    try:
        ...
        worksheet.update_cell(11, 1, url)
        for i, item in enumerate(stored_items, start=1):
            worksheet.update_cell(11, i + 1, item)
            print(f"Added item {i}: {item}")

        amsterdam_tz = pytz.timezone('Europe/Amsterdam')
        current_datetime_amsterdam = datetime.now(amsterdam_tz)

        worksheet.update_cell(11, len(stored_items) + 2, current_datetime_amsterdam.strftime("%Y-%m-%d"))
        worksheet.update_cell(11, len(stored_items) + 3, current_datetime_amsterdam.strftime("%H:%M:%S"))
