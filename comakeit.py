...
def main():
    url = 'https://www.comakeit.com/careers/'  # Updated URL

    try:
        driver = get_driver()
        driver.get(url)

        span_elements = driver.find_elements(By.CSS_SELECTOR, 'span.exad-modal-action-icon-right, span.exad-modal-action-icon-')  # Find <span> elements
        text_list = []

        for span in span_elements:
            span_text = span.text.strip()  # Get the text content of the <span> element
            if span_text:  # Check if the text is not empty
