...
def main():
    url = 'http://coachview.net/vacatures/'  # Updated URL

    try:
        driver = get_driver()
        driver.get(url)

        h3_elements = driver.find_elements(By.CSS_SELECTOR, 'h3.title')  # Find <h3> elements with the specified class
        stored_items = [h3.text.strip() for h3 in h3_elements]
