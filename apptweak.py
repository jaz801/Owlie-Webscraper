...
def main():
    url = 'https://jobs.apptweak.com/'  # Updated URL

    try:
        driver = get_driver()
        driver.get(url)

        a_elements = driver.find_elements(By.CSS_SELECTOR, 'a.sc-1w2f0ht-2.kpsNQL')  # Find <a> elements with class="sc-1w2f0ht-2 kpsNQL"
        ...
        for a_element in a_elements:
            ...
            if a_text:
                stored_items.append(a_text)
                print("<a> content:", a_text)
        ...
        worksheet.update_cell(14, 1, url)  # Update row 14, column 1 with the URL
        for i, item in enumerate(stored_items, start=1):
            worksheet.update_cell(14, i + 1, item)  # Update row 14, starting from column 2
            print(f"Added item {i}: {item}")

        amsterdam_tz = pytz.timezone('Europe/Amsterdam')
        current_datetime_amsterdam = datetime.now(amsterdam_tz)

        worksheet.update_cell(14, len(stored_items) + 2, current_datetime_amsterdam.strftime("%Y-%m-%d"))  # Update date
        worksheet.update_cell(14, len(stored_items) + 3, current_datetime_amsterdam.strftime("%H:%M:%S"))  # Update time
