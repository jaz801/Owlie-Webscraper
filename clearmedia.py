...
def main():
    url = 'https://www.clearmedia.be/nl/vacatures/'  # Updated URL

    try:
        driver = get_driver()
        driver.get(url)

        h2_elements = driver.find_elements(By.CSS_SELECTOR, 'h2.elementor-heading-title.elementor-size-default')  # Find <h2> elements with the specified class
        stored_items = []

        for h2 in h2_elements:
            h2_text = h2.text.strip()
            if h2_text and 'Neem contact met ons op:' not in h2_text:
