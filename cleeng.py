...
def main():
    url = 'https://cleeng.com/careers#open-positions'  # Updated URL

    try:
        driver = get_driver()
        driver.get(url)

        div_elements = driver.find_elements(By.CSS_SELECTOR, 'div.title-position')  # Find <div> elements with the specified class
        stored_items = []

        for div in div_elements:
            div_text = div.text.strip()
            if div_text and 'Neem contact met ons op:' not in div_text:
