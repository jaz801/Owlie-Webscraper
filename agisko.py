...
def main():
    url = 'https://www.agisko.be/careers'  # Updated URL

    try:
        driver = get_driver()
        driver.get(url)

        h4_elements = driver.find_elements(By.TAG_NAME, 'h4')  # Find h4 elements
        stored_items = []
        ...
        worksheet.update_cell(12, 1, url)
        for i, item in enumerate(stored_items, start=1):
            worksheet.update_cell(12, i + 1, item)
            print(f"Added item {i}: {item}")

        amsterdam_tz = pytz.timezone('Europe/Amsterdam')
        current_datetime_amsterdam = datetime.now(amsterdam_tz)

        worksheet.update_cell(12, len(stored_items) + 2, current_datetime_amsterdam.strftime("%Y-%m-%d"))
        worksheet.update_cell(12, len(stored_items) + 3, current_datetime_amsterdam.strftime("%H:%M:%S"))
